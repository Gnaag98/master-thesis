...
def plot(grid_size_x: int, grid_size_y: int, x: list, y: list,
    ...
    # Plot cell index associated with each particle at the particle positions.
    for i, text in enumerate(cell_indices):
        ax.plot(x[i], y[i], 'ko', markersize=10)
        ax.text(x[i], y[i], f'{text}', color='w', fontsize=7,
                horizontalalignment='center', verticalalignment='center')
...
def main():
    ...
    # Get indices of associated cells
    cell_indices = np.load(file_directory / 'associated_cells.npy')
    
    plot(grid_size_x, grid_size_y, x, y, cell_indices)
    plt.show()
